...
const SummaryModal = (props) => {
    ...
    const currentId = (Array.isArray(selectedParentStack) && selectedParentStack.length > 0) ? selectedParentStack[selectedParentStack.length - 1] : selectedSummaryParentId;
    ...
    // Breadcrumb scroll metrics to mirror HierarchicalObjectsScreen styling
                                ...
                                <FlatList
                                    ...
                                    renderItem={({ item }) => {
                                        ...
                                        if (item.type === 'group-member') {
                                            ...
                                            return (
                                                <TouchableOpacity key={`parent-group-${n.id}`} onPress={() => { setSelectedSummaryParentId(anchor); setSelectedParentStack([anchor]); setSelectedGroupKey(item.group_key || null); setPressedGroupKey(item.group_key || null); }} style={[AppStyles.card, AppStyles.cardGroupMember, { marginBottom: 6 }]}>
                                        ...
                                        return (
                                            <TouchableOpacity key={`parent-solo-${n.id}`} onPress={() => { setSelectedSummaryParentId(n.id); setSelectedParentStack([n.id]); setSelectedGroupKey(null); setSelectedGroupMembers([]); setPressedGroupKey(null); }} style={[AppStyles.card, { marginBottom: 8 }]}>
                                                    <View style={AppStyles.cardFlex}>
                                                        <View style={AppStyles.cardContent}>
                                                            <Text style={AppStyles.cardTitle}>{n.naam}</Text>
                                                            <Text style={AppStyles.cardSubtitle}>{propCount} eigenschappen</Text>
                                    ...
                                    ListFooterComponent={() => (
                                        leftListData && leftListData.length > (leftPage * leftPageSize) ? (
                        ...
                        <View style={{ flex: 1, paddingLeft: 12 }}>
                            {(!selectedSummaryParentId && selectedParentStack.length === 0) ? (
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    {/* Ancestor / Current / Descendant breadcrumb row */}
                                    {(() => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {parts.map((p, idx) => {
                                                    ...
                                                    // compute label for grouped nodes
                                            ...
                                            {/* Breadcrumb scrollbar (match HierarchicalObjectsScreen look) */}
                                        ...
                                        {currentNode && Array.isArray(currentNode.children) && currentNode.children.length > 0 ? (
                                            <FlatList
                                                ...
                                                renderItem={({ item }) => {
                                                    ...
                                                    return (
                                                        <TouchableOpacity key={`child-${ch.id}`} onPress={() => { setSelectedParentStack && setSelectedParentStack((prev) => [...(prev || []), ch.id]); setSelectedSummaryParentId && setSelectedSummaryParentId(ch.id); }} style={[AppStyles.card, { marginBottom: 8 }]}>
                                                            <View style={AppStyles.cardContent}>
                                                                <Text style={AppStyles.cardTitle}>{ch.naam}</Text>
                                                                <Text style={AppStyles.cardSubtitle}>{propCount} eigenschappen</Text>
                                                            </View>
